...
export default function AgentPage() {
  ...
  const [apiDesc, set_apiDesc] = useState('');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/v1/agents/api-mock-generator/run`, {
        method: 'POST',
  ...
  return (
    <AgentPageLayout agent={agent}>
      <form onSubmit={handleSubmit} className='flex flex-col gap-5'>
        ...
        <div>
          <label htmlFor="input1" style={labelStyle}>API Description</label>
          <textarea
            id="input1"
            placeholder="e.g. A REST API for a task management app."
            value={apiDesc}
            onChange={(e) => set_apiDesc(e.target.value)}
        ...
        <button
          type='submit'
          disabled={loading || !groqApiKey.trim() || !apiDesc.trim()}
