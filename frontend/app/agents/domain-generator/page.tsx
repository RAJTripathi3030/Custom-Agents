...
export default function AgentPage() {
  ...
  const [projectDesc, set_projectDesc] = useState('');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/v1/agents/domain-generator/run`, {
        method: 'POST',
  ...
  return (
    <AgentPageLayout agent={agent}>
      <form onSubmit={handleSubmit} className='flex flex-col gap-5'>
        ...
        <div>
          <label htmlFor="input1" style={labelStyle}>Project Description</label>
          <textarea
            id="input1"
            placeholder="e.g. An AI-powered email client."
            value={projectDesc}
            onChange={(e) => set_projectDesc(e.target.value)}
        ...
        <button
          type='submit'
          disabled={loading || !groqApiKey.trim() || !projectDesc.trim()}
