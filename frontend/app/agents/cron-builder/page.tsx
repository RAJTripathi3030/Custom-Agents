...
export default function AgentPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/v1/agents/cron-builder/run`, {
        method: 'POST',
  ...
  return (
    <AgentPageLayout agent={agent}>
      <form onSubmit={handleSubmit} className='flex flex-col gap-5'>
        ...
        <div>
          <label htmlFor="input1" style={labelStyle}>Schedule Description</label>
          <input
            id="input1"
            placeholder="e.g. every weekday at 9am"
            value={schedule}
            onChange={(e) => set_schedule(e.target.value)}
        ...
        <button
          type='submit'
          disabled={loading || !groqApiKey.trim() || !schedule.trim()}
